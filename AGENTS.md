...
tests/         End-to-end tests using testcontainers (docker compose)
...
Three workflow files, seven jobs total: `lint` (ruff on all 4 packages), `processing-unit-tests`, `processing-integration-tests`, `mcp-server-unit-tests`, `mcp-server-integration-tests`, `e2e-tests`, and `docker` (`docker compose build`). Runs on every push and PR.
...
- **Unit tests**: `mcp_server/tests/unit/` — run with `uv run --directory mcp_server pytest tests/unit -v`
- **Integration tests**: `mcp_server/tests/integration/` — run with `uv run --directory mcp_server pytest tests/integration -v`
- **End-to-end tests**: `tests/e2e/` — run with `uv run --directory tests pytest e2e -v` (spins up full stack via testcontainers)

## Architecture notes

- **Processing** accepts uploads via `POST /process_document/` and runs the Docling pipeline in the background.
- **Processing** also exposes `POST /add_documents/` for direct insertion into ChromaDB collections (used by tests).
